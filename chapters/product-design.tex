...
Creato da \textit{Evan You}, Vue.js è apprezzato per la sua semplicità d'uso e flessibilità. Con un sistema di reattività basato su un modello di oggetti e dipendenze, 
...
\textit{Vite.js} è stato progettato per essere veloce, semplice da utilizzare e facile da configurare. La sua velocità è dovuta al fatto che utilizza la tecnica dell'\textit{ESM (ECMAScript Modules)}
...
Può essere considerato un preprocessore \textit{CSS}, in quanto viene compilato in CSS prima di essere interpretato dal \textit{browser}. \textit{Sass} inoltre permette di utilizzare funzionalità non disponibili in \textit{CSS} nativo, offrendo una serie di funzioni, variabili, \textit{mixin} e altro.\\
...
\textit{Vue-router} è una libreria per la gestione delle \textit{route} per le applicazione \textit{Vue.js}. Permette di definire le \textit{route} dell'applicazione e di navigare tra le pagine.
...
\textit{AWS CodeCommit} rappresenta un servizio di \textit{hosting} di \textit{repository} altamente scalabile, che è gestito all'interno dell'ecosistema di \textit{Amazon Web Services (AWS)}. 
...
Libreria \textit{THRON} contenente i componenti del \textit{design system} aziendale. Questa libreria è stata utilizzata per la creazione di componenti comuni come bottoni del portale.
...
Questo è il primo step del processo di \textit{deploy}, in quanto viene eseguito la \textit{build} sia del \textit{middleware} che del portale.
...
In breve questo costrutto genera il template di \textit{AWS CloudFront} ed esegui il \textit{deploy} degli \textit{asset} statici su un \textit{bucket S3}.\\
...
\begin{itemize}
  \item La cartella \textit{src} che contiene il codice sorgente dell'applicazione, dove vengono specificati i vari \textit{endpoint} che utilizzo sulla parte frontend, lo \textit{script} per il \textit{setting}
...
L'architettura di \textit{Vue.js} è basata sul pattern architetturale MVVM (Model-View-ViewModel), che è una variante del pattern MVC (Model-View-Controller), dove:
\begin{itemize}
